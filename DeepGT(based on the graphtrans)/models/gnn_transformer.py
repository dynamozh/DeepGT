...
class GNNTransformer(BaseModel):
    ...
    def __init__(self, num_tasks, node_encoder, edge_encoder_cls, args):
        ...
        self.is_inductive_node = args.is_inductive_node
    ...
    def forward(self, batched_data, perturb=None):
        ...
        if self.is_inductive_node:
            max_num_nodes, num_batch, feature_dim = transformer_out.shape
            # num_nodes = max_num_nodes - src_padding_mask.sum(dim=1)  # 计算每个batch实际的节点数（不知道为啥用这个计算的部分对，部分错）

            out = torch.zeros(sum(num_nodes), feature_dim, device=transformer_out.device)

            idx = 0
            for i in range(num_batch):
                out[idx:idx + num_nodes[i]] = transformer_out[-num_nodes[i]:, i]
                idx += num_nodes[i]
            out = self.graph_pred_linear(out)
            return out
