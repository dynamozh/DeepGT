...
def main():
    ...
    parser.add_argument('--data_root', type=str, default='/home/zn/data/ogb')
    ...
    group.add_argument('--model_type', type=str, default='gnn-transformer', help='gnn|pna|gnn-transformer')
    ...
    group.add_argument('--gnn_emb_dim', type=int, default=128,
    ...
    group.add_argument('--is_inductive_node', type=bool, default=False)
    # fmt: on
    ...
    def run(run_id):
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if args.scheduler == "plateau":
                valid_perf = eval(model, device, valid_loader, evaluator, args.is_inductive_node)
            ...
            if epoch > args.start_eval and epoch % args.test_freq == 0 or epoch in [1, args.epochs]:
                ...
                with torch.no_grad():
                    train_perf = eval(model, device, train_loader_eval, evaluator, args.is_inductive_node)
                    if args.scheduler != "plateau":
                        valid_perf = eval(model, device, valid_loader, evaluator, args.is_inductive_node)
                    test_perf = eval(model, device, test_loader, evaluator, args.is_inductive_node)
        ...
        model.load_state_dict(state_dict["model"])
        best_valid_perf = eval(model, device, valid_loader, evaluator, args.is_inductive_node)
        best_test_perf = eval(model, device, test_loader, evaluator, args.is_inductive_node)
