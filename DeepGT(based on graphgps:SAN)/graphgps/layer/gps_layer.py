...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        # self.keep_local_global_a = nn.Parameter(torch.Tensor([1.0]))  # Initialize with 1.0 (default to keep the output)
        # self.keep_local_global_b = nn.Parameter(torch.Tensor([1.0]))  # Initialize with 1.0 (default to keep the output)
        # self.keep_residual_layer = nn.Parameter(torch.Tensor([1.0]))  # Initialize with 1.0 (default to keep the layer)
        # self.keep_ff_layer = nn.Parameter(torch.Tensor([1.0]))  # Initialize with 1.0 (default to keep the layer)
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            # h_local = h_local * self.keep_local_global_a  # Apply the learned parameter
        ...
        if self.self_attn is not None:
            ...
            # h_attn = h_in1 + h_attn * self.keep_residual_layer   # Residual connection(Apply the learned parameter)
            ...
            # h_attn = h_attn * self.keep_local_global_b  # Apply the learned parameter
        ...
        # h = h + self._ff_block(h) * self.keep_ff_layer  # Apply the learned parameter
        if self.layer_norm:
