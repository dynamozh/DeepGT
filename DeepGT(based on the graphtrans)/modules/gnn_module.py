...
class GNN_node(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, node_encoder, edge_encoder_cls, drop_ratio=0.5, JK="last", residual=False, gnn_type="gin"):
        ...
        # self.keep_gnn_layer = torch.nn.Parameter(torch.ones(num_layer))

    def forward(self, batched_data, perturb=None):
        x, edge_index, edge_attr, batch = batched_data.x, batched_data.edge_index, batched_data.edge_attr, batched_data.batch
        # 判断 edge_attr 是否已经是二维
        if edge_attr is not None and edge_attr.dim() != 2:
            # 如果不是二维，为 edge_attr 增加一个维度，使其变为二维
        ...
        for layer in range(self.num_layer):
            ...
            if self.residual:
                # h = h * self.keep_gnn_layer[layer] + h_list[layer]
                h += h_list[layer]
