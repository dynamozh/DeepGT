...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    keep_local_global_a_record = []
    keep_local_global_b_record = []
    keep_residual_layer_record = []
    keep_ff_layer_record = []
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        keep_local_global_a_record_inner = []
        keep_local_global_b_record_inner = []
        keep_residual_layer_record_inner = []
        keep_ff_layer_record_inner = []
        start_time = time.perf_counter()
        train_epoch(loggers[0], loaders[0], model, optimizer, scheduler,
                    cfg.optim.batch_accumulation)

        # Iterate through model layers and pull out stored para.
        for l_i, (name, module) in enumerate(model.model.layers.named_children()):
            if hasattr(module, 'keep_local_global_a'):
                keep_local_global_a_record_inner.append(module.keep_local_global_a.item())
            if hasattr(module, 'keep_local_global_b'):
                keep_local_global_b_record_inner.append(module.keep_local_global_b.item())
            if hasattr(module, 'keep_residual_layer'):
                keep_residual_layer_record_inner.append(module.keep_residual_layer.item())
            if hasattr(module, 'keep_ff_layer'):
                keep_ff_layer_record_inner.append(module.keep_ff_layer.item())
        keep_local_global_a_record.append(keep_local_global_a_record_inner)
        keep_local_global_b_record.append(keep_local_global_b_record_inner)
        keep_residual_layer_record.append(keep_residual_layer_record_inner)
        keep_ff_layer_record.append(keep_ff_layer_record_inner)
    ...
    torch.save(keep_local_global_a_record, './para/keep_local_global_a_record.pt')
    torch.save(keep_local_global_b_record, './para/keep_local_global_b_record.pt')
    torch.save(keep_residual_layer_record, './para/keep_residual_layer_record.pt')
    torch.save(keep_ff_layer_record, './para/keep_ff_layer_record.pt')
